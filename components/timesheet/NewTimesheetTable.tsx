...
import { DayType, WorkDay } from '@/types/workday'
...
export default function NewTimesheetTable({
  workDays,
  currentDate,
  isLoading,
  onSave,
  ...
  useEffect(() => {
    const newEditableData: {[key: string]: any} = {}
    ...
    try {
      ...
      for (const employee of employeesWithoutEntries) {
        try {
          ...
          console.error(`Ошибка создания записи для сотрудника ${employee.name}:`, err)
        }
      }
